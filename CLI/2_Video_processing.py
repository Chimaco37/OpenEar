...
def process_file(full_run_with_extension, pic_path, model_path, image_undistort_parameters):
    ...
    # Stitch images
    run_command(
        ["magick", f"{full_run}_undistort*.png", "-crop", "1x1440+536+0", "+repage", f"{full_run}_pixel%03d.png"]
    )
    ...
    run_command([
        "magick", f"{full_run}_pixel*.png", "+append", "+repage", f"{full_run}_raw.png"
    ])
    ...
    # Crop raw image
    run_command([
        "magick", f"{full_run}_raw.png", "-crop", f"{resize_length}x1440+40+0", "+repage", f"{full_run}_cropped.png"
    ])
    delete_file(f"{full_run}_raw.png")

    # Final resize operation
    run_command([
        "magick", f"{full_run}_cropped.png", "-resize", "1920x1440!", f"{full_run}.png"
    ])
